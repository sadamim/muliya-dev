      ...
      <div className="relative bg-gradient-to-r from-amber-50 to-yellow-50 py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif text-gray-900 mb-4">Harams</h1>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">Discover our exquisite collection of traditional South Indian harams. From temple designs to diamond studded pieces, find the perfect long necklace for special occasions.</p>
          </div>
        </div>
      </div>
          ...
          {harams.map((haram) => (
            <div key={haram.id} onClick={() => handleProductClick(haram.id)} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col cursor-pointer">
              <div className="relative aspect-square overflow-hidden bg-gray-100 flex-shrink-0">
                <Image src={haram.image} alt={haram.name} fill className="object-cover transition-transform duration-500 group-hover:scale-110" />
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-xl font-bold text-[#E92247]">₹{haram.price.toLocaleString()}</span>
                  {haram.originalPrice && <span className="text-sm text-gray-400 line-through">₹{haram.originalPrice.toLocaleString()}</span>}
                </div>
